...
def buy_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✨ 𝗠𝟰𝗨_𝗣𝗿𝗲𝗺𝗶𝘂𝗺 (₹35/1d)", callback_data="plan_m4u")],
        [InlineKeyboardButton("📂 𝗨𝗻𝗹𝗶𝗺𝗶𝘁𝗲𝗱 𝗙𝗶𝗹𝗲'𝘀 (₹20/1d)", callback_data="plan_file")],
        [InlineKeyboardButton("🔥 𝗕𝗼𝘁𝗵 (₹50/1d)", callback_data="plan_both")],
    ])
...
async def plan_cmd(client, message):
    ...
    for doc in docs:
        ...
        days_left = 1 - (datetime.date.today() - join_date).days
...
async def select_plan(client, cq: CallbackQuery):
    ...
    if existing:
        await cq.answer()
        await cq.message.reply_text(f"⚠️ You already purchased {PLAN_NAMES[plan]}. Please wait until it expires.")
        return
...
async def handle_screenshot(client, message):
    ...
    await client.send_message(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Approve M4U", callback_data=f"approve_{user_id}_m4u")],
            [InlineKeyboardButton("✅ Approve Files", callback_data=f"approve_{user_id}_file")],
