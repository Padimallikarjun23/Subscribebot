...
CHANNEL_2 = -1001234567890   # 🔴 Replace with your second channel ID
...
# ==== EXPIRY LIMIT (days) ====
DAYS_LIMIT = 1   # ⚡ set to 1 for testing, change to 30 for production
...
# ==== CALLBACK HANDLER (all buttons here) ====
@app.on_callback_query()
async def all_callbacks(client, cq: CallbackQuery):
    await cq.answer()  # must always answer
    ...
    if cq.data == "plan":
        ...
        doc = members.find_one({"user_id": user_id, "join_date": {"$ne": "PENDING"}})
        if not doc:
            await cq.message.reply_text("❌ You don't have an active plan.")
        else:
            await cq.message.reply_text(
                f"💰 Your Current Plan:\n\n"
                f"Plan: {PLAN_NAMES[doc['plan']]}\n"
                f"Join Date: {doc['join_date']}"
            )
        return
    ...
    if cq.data.startswith("plan_"):
        ...
        if existing:
            await cq.message.reply_text(f"⚠️ You already purchased {PLAN_NAMES[plan]} plan. Wait until it expires.")
            return
