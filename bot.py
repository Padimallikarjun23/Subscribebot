...
CHANNEL_2 = -100  # if you want future use
...
# ==== PLAN NAMES (for display only) ====
PLAN_NAMES = {
    "malli": "✨ 𝗠𝗮𝗹𝗹𝗶𝟒𝗨_𝗣𝗿𝗲𝗺𝗶𝘂𝗺",
...
def buy_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{PLAN_NAMES['malli']} (₹{PLAN_PRICES['malli']}/30d)", callback_data="plan_malli")],
        [InlineKeyboardButton(f"{PLAN_NAMES['files']} (₹{PLAN_PRICES['files']}/30d)", callback_data="plan_files")],
...
async def stats(client, message):
    ...
    malli = members.count_documents({"plan": "malli"})
    files = members.count_documents({"plan": "files"})
    both = members.count_documents({"plan": "both"})

    await message.reply_text(
        f"📊 Subscription Stats:\n\n"
        f"👥 Total Unique Users: {len(total)}\n"
        f"{PLAN_NAMES['malli']}: {malli}\n"
        f"{PLAN_NAMES['files']}: {files}\n"
...
# ==== PLAN SELECTION (Mallli/Files/Both) ====
...
async def select_plan(client, cq: CallbackQuery):
    ...
    await cq.message.edit_text(
        ...
        "(Admin will verify via @M4U_Admin_Bot)"
    )
    members.update_one(
...
async def handle_screenshot(client, message):
    ...
    await client.send_message(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(f"✅ Approve {PLAN_NAMES['malli']}", callback_data=f"approve_{user_id}_malli")],
            [InlineKeyboardButton(f"✅ Approve {PLAN_NAMES['files']}", callback_data=f"approve_{user_id}_files")],
